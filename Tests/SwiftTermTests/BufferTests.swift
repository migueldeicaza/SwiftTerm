...
final class BufferTests: XCTestCase, TerminalDelegate {
    ...
    func testIssue256_YBaseResetOnClear() {
        let terminal = Terminal(delegate: self, options: TerminalOptions(cols: 80, rows: 25))

        // Switch to alternate buffer
        terminal.feed(text: "\u{1b}[?1049h")
        XCTAssertTrue(terminal.buffer === terminal.altBuffer, "Should be in alt buffer")
        ...
        XCTAssertTrue(terminal.buffer === terminal.normalBuffer, "Should be in normal buffer")

        // The fix: yBase should be reset to 0 after clear()
        XCTAssertEqual(terminal.altBuffer.yBase, 0,
            "yBase should be reset to 0 after clear(). Was \(altYBaseBeforeSwitch) before switch.")

        // Switch back to alt buffer
        terminal.feed(text: "\u{1b}[?1049h")
        XCTAssertTrue(terminal.buffer === terminal.altBuffer, "Should be in alt buffer again")

        // yBase should still be 0 for the fresh alt buffer
        XCTAssertEqual(terminal.buffer.yBase, 0, "yBase should be 0 for fresh alt buffer")
        ...
        XCTAssertTrue(true, "reverseIndex did not crash")
    }
    ...
    func testIssue256_ReverseIndexWithInvalidYBase() {
        let terminal = Terminal(delegate: self, options: TerminalOptions(cols: 80, rows: 25))
        ...
        XCTAssertEqual(buffer.yBase, 0, "yBase should start at 0")
        XCTAssertEqual(buffer.lines.count, 25, "Alt buffer should have exactly rows lines")
        ...
        XCTAssertEqual(buffer.y, 0, "Cursor should be at row 0")
        XCTAssertEqual(buffer.scrollTop, 0, "scrollTop should be 0")
    ...
    func testBufferClearResetsYBase() {
        ...
        XCTAssertEqual(terminal.altBuffer.yBase, 0, "yBase should be 0 after clear")
        XCTAssertEqual(terminal.altBuffer.yDisp, 0, "yDisp should be 0 after clear")
        XCTAssertEqual(terminal.altBuffer.x, 0, "x should be 0 after clear")
        XCTAssertEqual(terminal.altBuffer.y, 0, "y should be 0 after clear")
        XCTAssertEqual(terminal.altBuffer.linesTop, 0, "linesTop should be 0 after clear")
    }
    ...
    func testBufferClearMustResetYBase() {
        let terminal = Terminal(delegate: self, options: TerminalOptions(cols: 80, rows: 25))
        ...
        // Verify our setup
        XCTAssertEqual(terminal.altBuffer.yBase, 50, "Setup: yBase should be 50")
    ...
    func testIssue256_CrashConditionPrevented() {
        let terminal = Terminal(delegate: self, options: TerminalOptions(cols: 80, rows: 25))
    ...
    func testDirectClearResetsYBase() {
        ...
        XCTAssertEqual(terminal.altBuffer.yBase, 0, "yBase should be 0 after direct clear()")
        XCTAssertEqual(terminal.altBuffer.yDisp, 0, "yDisp should be 0 after direct clear()")
        XCTAssertEqual(terminal.altBuffer.x, 0, "x should be 0 after direct clear()")
        XCTAssertEqual(terminal.altBuffer.y, 0, "y should be 0 after direct clear()")
        XCTAssertEqual(terminal.altBuffer.linesTop, 0, "linesTop should be 0 after direct clear()")
        XCTAssertEqual(terminal.altBuffer.scrollTop, 0, "scrollTop should be 0 after direct clear()")
        XCTAssertEqual(terminal.altBuffer.scrollBottom, 24, "scrollBottom should be rows-1 after direct clear()")
    }
    ...
    func testRapidBufferSwitches() {
        ...
        for i in 0..<20 {
            // Switch to alt buffer
            terminal.feed(text: "\u{1b}[?1049h")
            XCTAssertTrue(terminal.buffer === terminal.altBuffer, "Iteration \(i): Should be in alt buffer")
            ...
            terminal.feed(text: "\u{1b}[?1049l")
            XCTAssertTrue(terminal.buffer === terminal.normalBuffer, "Iteration \(i): Should be in normal buffer")

            // Verify alt buffer was properly cleared
            XCTAssertEqual(terminal.altBuffer.yBase, 0, "Iteration \(i): yBase should be reset after switch")
        ...
        terminal.feed(text: "\u{1b}[?1049h")
        XCTAssertEqual(terminal.buffer.yBase, 0, "yBase should be 0 after all switches")

        // reverseIndex should work without crashing
        terminal.feed(text: "\u{1b}[1;1H")  // Move to top
        terminal.feed(text: "\u{1b}M")       // Reverse index
        XCTAssertTrue(true, "Survived rapid buffer switches")
    }
    ...
    func testBufferSwitchDuringScrolling() {
        ...
        XCTAssertEqual(terminal.altBuffer.yBase, 0, "yBase should be reset")

        // Switch back to alt and verify it works
        terminal.feed(text: "\u{1b}[?1049h")
        XCTAssertEqual(terminal.buffer.yBase, 0, "Fresh alt buffer should have yBase=0")

        // Operations should work without crashing
        terminal.feed(text: "\u{1b}[3;1H")   // Move to scroll region top
        terminal.feed(text: "\u{1b}M")        // Reverse index

        XCTAssertTrue(true, "Buffer switch during scrolling handled correctly")
    }

    /// Test scroll() with corrupted yBase triggers defensive guard
    func testScrollWithCorruptedYBase() {
        let terminal = Terminal(delegate: self, options: TerminalOptions(cols: 80, rows: 25))
        ...
        // Should not crash
        XCTAssertTrue(true, "scroll() handled corrupted yBase without crashing")
    }

    static var allTests = [
        ("testIssue256_YBaseResetOnClear", testIssue256_YBaseResetOnClear),
        ("testIssue256_ReverseIndexWithInvalidYBase", testIssue256_ReverseIndexWithInvalidYBase),
        ...
        ("testScrollWithCorruptedYBase", testScrollWithCorruptedYBase),
    ]
}
