...
//                #endif
    ...
    /// Reports that `columns` cells were shifted right within `left...right` on
    /// one absolute row, so anchors into that row can move with their cells.
    /// Insert mode does this per wide character written, below the level the
    /// terminal's own CSI handlers see.
    ...
    func insertCharacter(_ charData: CharData, resolvePayload: () -> TinyAtom?) {
        ...
        if insertMode {
            ...
            bufferRow.insertCells (pos: _x, n: chWidth, rightMargin: rightMargin, fillData: empty)
